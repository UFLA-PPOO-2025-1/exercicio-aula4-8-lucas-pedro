...
public class Campo {
    ...
    public Localizacao localizacaoVizinhaLivre() {
        // As localizações livres disponíveis.
        List<Localizacao> livres = localizacoesTotalLivres();
        if (livres.size() > 0) {
            return livres.get(rand.nextInt(0, livres.size()));
        } else {
            return null;
        }
    }
